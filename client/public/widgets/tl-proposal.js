...
(function() {
  ...
  var css = '\
    ...
    .tl-prop-sig-input { width: 100%; padding: 16px; font-size: 24px; font-family: "Brush Script MT", cursive; border: 1px solid #e5e7eb; border-radius: 8px; margin-bottom: 16px; box-sizing: border-box; }\
          ...
          <div class="tl-prop-section">\
            ...
            <div class="tl-prop-total">\
              <div class="tl-prop-total-row">\
                <span class="tl-prop-total-label">Total:</span>\
                <span class="tl-prop-total-value">' + formatCurrency(total) + '</span>\
              </div>\
            </div>\
          </div>\
          <div class="tl-prop-section">\
            <div class="tl-prop-section-title">Terms & Conditions</div>\
            <p class="tl-prop-terms">' + proposal.terms + '</p>\
          </div>\
          <div class="tl-prop-signature">\
            <div class="tl-prop-sig-title">Sign to Accept This Proposal</div>\
            <input type="text" class="tl-prop-sig-input" id="tl-signature" placeholder="Type your signature here">\
            <div class="tl-prop-sig-line">\
              <div class="tl-prop-sig-field">\
                <label class="tl-prop-label">Printed Name</label>\
                <input type="text" id="tl-printed-name" value="' + proposal.customer.name + '">\
              </div>\
              <div class="tl-prop-sig-field">\
                <label class="tl-prop-label">Date</label>\
                <input type="text" id="tl-sign-date" value="' + new Date().toLocaleDateString() + '" readonly>\
              </div>\
            </div>\
            <button class="tl-prop-btn" id="tl-accept">Accept & Sign Proposal</button>\
          </div>\
        </div>\
        <div class="tl-prop-powered">Powered by <a href="https://tlid.io" target="_blank">TrustLayer</a></div>\
      </div>\
    ';
    document.getElementById('tl-accept').addEventListener('click', function() {
      var signature = document.getElementById('tl-signature').value.trim();
      var printedName = document.getElementById('tl-printed-name').value.trim();
      if (!signature) {
        alert('Please sign by typing your name in the signature field');
        return;
      }
      var signData = {
      ...
      sendWebhook(signData);
      if (window.TLAnalytics) {
        window.TLAnalytics.track('proposal_signed', signData);
      }
      container.querySelector('.tl-prop-container').innerHTML = '\
        <div class="tl-prop-success">\
          ...
          <p style="color:#6b7280;">Thank you for accepting this proposal. A confirmation has been sent to your email.</p>\
          <p style="color:#6b7280;margin-top:16px;font-size:14px;">Proposal #' + proposal.id + '</p>\
        </div>\
        <div class="tl-prop-powered">Powered by <a href="https://tlid.io" target="_blank">TrustLayer</a></div>\
      ';
    });
